...
export class OverflowList extends DeclarativeShadowElement {
  ...
  #reflowItems = (lastVisibleElement) => {
    ...
    const overflowingElements = elements.filter(
      (element) => !visibleElements.includes(element),
    );
    const [firstOverflowingElement] = overflowingElements;
    const hasOverflow = overflowingElements.length > 0;
    const placeholderWidth = firstOverflowingElement
      ? firstOverflowingElement.clientWidth
      : 0;

    // Move the elements to the correct slot
    for (const element of elements) {
      element.slot = overflowingElements.includes(element)
        ? overflowSlot.name
        : defaultSlot.name;
    }
