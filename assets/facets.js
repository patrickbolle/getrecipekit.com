...
class FacetStatusComponent extends Component {
  ...
  updatePriceSummary(minInput, maxInput) {
    ...
    facetStatus.innerHTML = `${this.#formatMoney(
      minInputNum,
    )}–${this.#formatMoney(maxInputNum)}`;
  }
  ...
  #parseCents(value, fallback = '0') {
    const parts = value
      ? value.trim().split(/[^0-9]/)
      : (parseInt(fallback, 10) / 100).toString();
    const [wholeStr, fractionStr, ...rest] = parts;
    if (typeof wholeStr !== 'string' || rest.length > 0)
